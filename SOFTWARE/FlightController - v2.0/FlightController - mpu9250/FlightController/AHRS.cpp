...
float AHRS::pitchBias = 0;
float AHRS::rollBias = 0;

float AHRS::rawGyroPitch; 
float AHRS::rawGyroRoll;
float AHRS::gyroPitch; 
float AHRS::gyroRoll;
float AHRS::accelPitch; 
float AHRS::accelRoll;

float AHRS::outKalmanPitch;
float AHRS::outKalmanRoll;
float AHRS::outCompPitch;
float AHRS::outCompRoll;

float AHRS::pitchAngle; 
float AHRS::rollAngle;
...
bool AHRS::begin(){
	...
	LOG("Pitch bias: "); LOGln(pitchBias);
	LOG("Roll bias: "); LOGln(rollBias);

	// Calibration for Acceleromer offset
	// AHRS::AccelCalibration();

	// Initialize Gyro values with accelerometer values 
...
float AHRS::getPitch(){ return (pitchAngle - pitchBias); }

float AHRS::getGyroRoll(){ return rawGyroRoll; }

float AHRS::getRoll(){ return (rollAngle - rollBias); }
...
void AHRS::debug(){
	// LOG(imu.yaw);	LOG(TAB);
	LOG(pitchAngle);	LOG(TAB);
	LOG(rollAngle);	LOG(NEW_LINE);
}
