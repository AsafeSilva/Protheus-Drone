...
// ***************************************************************************************
static void waitActivation(volatile uint32_t *throttle, uint32_t throttleThreshold, volatile uint32_t *yaw, uint32_t yawThreshold){
	...
	while(true){
		...
		if((*throttle < throttleThreshold) && (*yaw > yawThreshold))
			break;
	}
	DroneState = DISARMED;
}
...
// ***************************************************************************************
static void droneChangeState(uint32_t throttle, uint32_t throttleMin, uint32_t throttleMax, uint32_t yaw, uint32_t yawMin, uint32_t yawMax){
	if(DroneState == DISARMED){

		if((throttle < throttleMin+1000) && (yaw > yawMax-1000)){
			LOGln(DroneState != ARMED ? "STATE: ARMED!" : "");
			DroneState = ARMED;
		}else if((throttle > throttleMax-1000) && (yaw > yawMax-1000)){
